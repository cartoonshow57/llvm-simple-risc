...
//===----------------------------------------------------------------------===//
//
// This file implements the CGP1TargetLowering class.
//
//===----------------------------------------------------------------------===//
...
static cl::opt<int> CGP1LowerConstantMulThreshold(
    "cgp1-constant-mul-threshold", cl::Hidden,
...
CGP1TargetLowering::CGP1TargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(CGP1::SP);
...
//===----------------------------------------------------------------------===//
//                       CGP1 Inline Assembly Support
//===----------------------------------------------------------------------===//

Register CGP1TargetLowering::getRegisterByName(
...
CGP1TargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1)
    ...
    switch (Constraint[0]) {
    case 'r': // GENERAL_REGS
      return std::make_pair(0U, &CGP1::GPRRegClass);
    default:
      break;
    }
...
CGP1TargetLowering::getSingleConstraintMatchWeight(
...
void CGP1TargetLowering::LowerAsmOperandForConstraint(
...
#include "CGP1GenCallingConv.inc"

static unsigned NumFixedArgs;
static bool CC_CGP132_VarArg(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  if (ValNo < NumFixedArgs) {
    return CC_CGP132(ValNo, ValVT, LocVT, LocInfo, ArgFlags, State);
  }
...
SDValue CGP1TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // CGP1 target does not yet support tail call optimization.
  IsTailCall = false;
...
SDValue CGP1TargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  CGP1MachineFunctionInfo *CGP1MFI = MF.getInfo<CGP1MachineFunctionInfo>();
  ...
  if (CallConv == CallingConv::Fast) {
    CCInfo.AnalyzeFormalArguments(Ins, CC_CGP132_Fast);
  } else {
    CCInfo.AnalyzeFormalArguments(Ins, CC_CGP132);
  }
  ...
  // The CGP1 ABI for returning structs by value requires that we copy
  // the sret argument into rv for the return. Save the argument into
  // a virtual register so that we can access it from the return points.
  if (MF.getFunction().hasStructRetAttr()) {
    Register Reg = CGP1MFI->getSRetReturnReg();
    if (!Reg) {
      Reg = MF.getRegInfo().createVirtualRegister(getRegClassFor(MVT::i32));
      CGP1MFI->setSRetReturnReg(Reg);
    }
  ...
  if (IsVarArg) {
    ...
    CGP1MFI->setVarArgsFrameIndex(FI);
  }

  return Chain;
}

bool CGP1TargetLowering::CanLowerReturn(
  ...
  return CCInfo.CheckReturn(Outs, RetCC_CGP132);
}
...
CGP1TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_CGP132);
  ...
  if (DAG.getMachineFunction().getFunction().hasStructRetAttr()) {
    ...
    Chain = DAG.getCopyToReg(Chain, DL, CGP1::RV, Val, Glue);
    Glue = Chain.getValue(1);
    RetOps.push_back(
        DAG.getRegister(CGP1::RV, getPointerTy(DAG.getDataLayout())));
  }

  RetOps[0] = Chain; // Update chain

  unsigned Opc = CGP1ISD::RET_GLUE;
  if (Glue.getNode())
...
SDValue CGP1TargetLowering::LowerCCCCallTo(
  ...
  if (NumFixedArgs)
    CCInfo.AnalyzeCallOperands(Outs, CC_CGP132_VarArg);
  else {
    if (CallConv == CallingConv::Fast)
      CCInfo.AnalyzeCallOperands(Outs, CC_CGP132_Fast);
    else
      CCInfo.AnalyzeCallOperands(Outs, CC_CGP132);
  }
  ...
  for (unsigned I = 0, J = 0, E = ArgLocs.size(); I != E; ++I) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (StackPtr.getNode() == nullptr)
        StackPtr = DAG.getCopyFromReg(Chain, DL, CGP1::SP,
  ...
  uint8_t OpFlag = CGP1II::MO_NO_FLAG;
  if (G) {
  ...
  Chain = DAG.getNode(CGP1ISD::CALL, DL, NodeTys,
...
SDValue CGP1TargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_CGP132);
...
static LPCC::CondCode IntCondCCodeToICC(SDValue CC, const SDLoc &DL,
  ...
  // and CGP1 only supports integer comparisons, so only provide definitions
  // for them.
  switch (SetCCOpcode) {
  case ISD::SETEQ:
    return LPCC::ICC_EQ;
...
SDValue CGP1TargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Glue = DAG.getNode(CGP1ISD::SET_FLAG, DL, MVT::Glue, LHS, RHS);

  return DAG.getNode(CGP1ISD::BR_CC, DL, Op.getValueType(), Chain, Dest,
                     TargetCC, Glue);
}

SDValue CGP1TargetLowering::LowerMUL(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (InstrRequired > CGP1LowerConstantMulThreshold)
    return SDValue();
...
SDValue CGP1TargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(CGP1ISD::SETCC, DL, Op.getValueType(), TargetCC, Glue);
}

SDValue CGP1TargetLowering::LowerSELECT_CC(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  return DAG.getNode(CGP1ISD::SELECT_CC, DL, Op.getValueType(), TrueV, FalseV,
                     TargetCC, Glue);
}

SDValue CGP1TargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  CGP1MachineFunctionInfo *FuncInfo = MF.getInfo<CGP1MachineFunctionInfo>();
...
SDValue CGP1TargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDValue Chain = Op.getOperand(0);
  ...
  // For CGP1, the outgoing memory arguments area should be on top of the
  ...
  SDValue ArgAdjust = DAG.getNode(CGP1ISD::ADJDYNALLOC, DL, MVT::i32, Sub);
...
SDValue CGP1TargetLowering::LowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
SDValue CGP1TargetLowering::LowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
  ...
  EVT VT = Op.getValueType();
  SDLoc DL(Op);
  SDValue FrameAddr = DAG.getCopyFromReg(DAG.getEntryNode(), DL, CGP1::FP, VT);
  unsigned Depth = Op.getConstantOperandVal(0);
  while (Depth--) {
...
SDValue CGP1TargetLowering::LowerConstantPool(SDValue Op,
  ...
  const CGP1TargetObjectFile *TLOF =
      static_cast<const CGP1TargetObjectFile *>(
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small ||
    ...
    SDValue Small = DAG.getTargetConstantPool(
        C, MVT::i32, N->getAlign(), N->getOffset(), CGP1II::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(CGP1::R0, MVT::i32),
                       DAG.getNode(CGP1ISD::SMALL, DL, MVT::i32, Small));
  } else {
    uint8_t OpFlagHi = CGP1II::MO_ABS_HI;
    uint8_t OpFlagLo = CGP1II::MO_ABS_LO;
    ...
    Hi = DAG.getNode(CGP1ISD::HI, DL, MVT::i32, Hi);
    Lo = DAG.getNode(CGP1ISD::LO, DL, MVT::i32, Lo);
...
SDValue CGP1TargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  if (TLOF->isGlobalInSmallSection(GO, getTargetMachine())) {
    SDValue Small = DAG.getTargetGlobalAddress(
        GV, DL, getPointerTy(DAG.getDataLayout()), Offset, CGP1II::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(CGP1::R0, MVT::i32),
                       DAG.getNode(CGP1ISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue CGP1TargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue CGP1TargetLowering::LowerJumpTable(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small) {
    SDValue Small = DAG.getTargetJumpTable(
        JT->getIndex(), getPointerTy(DAG.getDataLayout()), CGP1II::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(CGP1::R0, MVT::i32),
                       DAG.getNode(CGP1ISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue CGP1TargetLowering::LowerSHL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  ...
  // CGP1 shifts of larger than register sizes are wrapped rather than
...
SDValue CGP1TargetLowering::LowerSRL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  ...
  // Note: This takes advantage of CGP1's shift behavior to avoid needing to
...
SDValue CGP1TargetLowering::PerformDAGCombine(SDNode *N,
...
void CGP1TargetLowering::computeKnownBitsForTargetNode(
