//===-- CGP1DelaySlotFiller.cpp - CGP1 delay slot filler ----------------===//
//
...
static cl::opt<bool>
    NopDelaySlotFiller("cgp1-nop-delay-filler", cl::init(false),
                       cl::desc("Fill CGP1 delay slots with NOPs."),
                       cl::Hidden);
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "CGP1 Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);

  bool runOnMachineFunction(MachineFunction &MF) override {
    const CGP1Subtarget &Subtarget = MF.getSubtarget<CGP1Subtarget>();
...
// createCGP1DelaySlotFillerPass - Returns a pass that fills in delay
// slots in CGP1 MachineFunctions
FunctionPass *
llvm::createCGP1DelaySlotFillerPass(const CGP1TargetMachine & /*tm*/) {
  return new Filler();
}
  ...
  for (MachineBasicBlock::instr_iterator I = MBB.instr_begin();
    ...
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (I->getOpcode() == CGP1::RET) {
        ...
        assert(RI->getOpcode() == CGP1::LDW_RI && RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == CGP1::FP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == CGP1::FP &&
               RI->getOperand(2).isImm() && RI->getOperand(2).getImm() == -8);
        ++RI;
        assert(RI->getOpcode() == CGP1::ADD_I_LO &&
               RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == CGP1::SP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == CGP1::FP);
        ...
        if (!NopDelaySlotFiller && findDelayInstr(MBB, I, J)) {
          ...
          BuildMI(MBB, std::next(I), DebugLoc(), TII->get(CGP1::NOP));
        }
        ++FilledSlots;
      }
