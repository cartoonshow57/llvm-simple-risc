//===-- CGP1AsmParser.cpp - Parse CGP1 assembly to MCInst instructions --===//
//
...
class CGP1AsmParser : public MCTargetAsmParser {
  // Parse operands
  std::unique_ptr<CGP1Operand> parseRegister(bool RestoreOnFailure = false);

  std::unique_ptr<CGP1Operand> parseImmediate();

  std::unique_ptr<CGP1Operand> parseIdentifier();
...
public:
  CGP1AsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
// CGP1Operand - Instances of this class represented a parsed machine
// instruction
struct CGP1Operand : public MCParsedAsmOperand {
  enum KindTy {
    TOKEN,
    REGISTER,
    IMMEDIATE,
  ...
  explicit CGP1Operand(KindTy Kind) : Kind(Kind) {}
...
public:
  ...
  bool isHiImm16() {
    ...
    if (const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(Imm.Value))
      return SymbolRefExpr->getSpecifier() == CGP1::S_ABS_HI;
  ...
  bool isLoImm16() {
    ...
    // Symbolic reference expression
    if (const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(Imm.Value))
      return SymbolRefExpr->getSpecifier() == CGP1::S_ABS_LO;
  ...
  bool isLoImm21() {
    ...
    // Symbolic reference expression
    if (const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(Imm.Value))
      return SymbolRefExpr->getSpecifier() == CGP1::S_None;
  ...
  void addLoImm16Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      ...
      assert(SymbolRefExpr && SymbolRefExpr->getSpecifier() == CGP1::S_ABS_LO);
#endif
...
#ifndef NDEBUG
      const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(getImm());
      assert(SymbolRefExpr && SymbolRefExpr->getSpecifier() == CGP1::S_ABS_HI);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
...
#ifndef NDEBUG
      const auto *SymbolRefExpr = dyn_cast<MCSpecifierExpr>(getImm());
      assert(SymbolRefExpr && SymbolRefExpr->getSpecifier() == CGP1::S_None);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
  ...
  static std::unique_ptr<CGP1Operand> CreateToken(StringRef Str, SMLoc Start) {
    auto Op = std::make_unique<CGP1Operand>(TOKEN);
  ...
  static std::unique_ptr<CGP1Operand> createReg(MCRegister Reg, SMLoc Start,
                                                 SMLoc End) {
    auto Op = std::make_unique<CGP1Operand>(REGISTER);
    Op->Reg.RegNum = Reg;
    Op->StartLoc = Start;
    Op->EndLoc = End;
    return Op;
  ...
  static std::unique_ptr<CGP1Operand> createImm(const MCExpr *Value,
                                                 SMLoc Start, SMLoc End) {
    auto Op = std::make_unique<CGP1Operand>(IMMEDIATE);
    Op->Imm.Value = Value;
    Op->StartLoc = Start;
    Op->EndLoc = End;
    return Op;
  ...
  static std::unique_ptr<CGP1Operand>
  MorphToMemImm(std::unique_ptr<CGP1Operand> Op) {
  ...
  static std::unique_ptr<CGP1Operand>
  MorphToMemRegReg(MCRegister BaseReg, std::unique_ptr<CGP1Operand> Op,
                   unsigned AluOp) {
  ...
  static std::unique_ptr<CGP1Operand>
  MorphToMemRegImm(MCRegister BaseReg, std::unique_ptr<CGP1Operand> Op,
                   unsigned AluOp) {
    const MCExpr *Imm = Op->getImm();
...
bool CGP1AsmParser::matchAndEmitInstruction(SMLoc IdLoc, unsigned &Opcode,
                                             OperandVector &Operands,
                                             MCStreamer &Out,
                                             uint64_t &ErrorInfo,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      ErrorLoc = ((CGP1Operand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IdLoc;
    }
...
std::unique_ptr<CGP1Operand>
CGP1AsmParser::parseRegister(bool RestoreOnFailure) {
  ...
  if (Lexer.getKind() == AsmToken::Identifier) {
    ...
    return CGP1Operand::createReg(Reg, Start, End);
  }
  if (PercentTok && RestoreOnFailure)
    Lexer.UnLex(*PercentTok);
  return nullptr;
...
bool CGP1AsmParser::parseRegister(MCRegister &RegNum, SMLoc &StartLoc,
                                   SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<CGP1Operand> Op = parseRegister(/*RestoreOnFailure=*/false);
  if (Op != nullptr)
    RegNum = Op->getReg();
  return (Op == nullptr);
}

ParseStatus CGP1AsmParser::tryParseRegister(MCRegister &Reg, SMLoc &StartLoc,
                                             SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<CGP1Operand> Op = parseRegister(/*RestoreOnFailure=*/true);
  if (Op == nullptr)
...
std::unique_ptr<CGP1Operand> CGP1AsmParser::parseIdentifier() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  const MCExpr *Res, *RHS = nullptr;
  auto Kind = CGP1::S_None;
  ...
  return CGP1Operand::createImm(Res, Start, End);
}

std::unique_ptr<CGP1Operand> CGP1AsmParser::parseImmediate() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  ...
  case AsmToken::Dot:
    if (!Parser.parseExpression(ExprVal))
      return CGP1Operand::createImm(ExprVal, Start, End);
    [[fallthrough]];
  default:
    return nullptr;
  }
...
unsigned CGP1AsmParser::parseAluOperator(bool PreOp, bool PostOp) {
  StringRef IdString;
  Parser.parseIdentifier(IdString);
  unsigned AluCode = LPAC::stringToCGP1AluCode(IdString);
...
bool CGP1AsmParser::parsePrePost(StringRef Type, int *OffsetValue) {
...
bool shouldBeSls(const CGP1Operand &Op) {
  ...
  if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Op.getImm())) {
    ...
    return (LHSSymbolRefExpr &&
            LHSSymbolRefExpr->getSpecifier() == CGP1::S_None);
  }
  return false;
}

// Matches memory operand. Returns true if error encountered.
ParseStatus CGP1AsmParser::parseMemoryOperand(OperandVector &Operands) {
  ...
  if (Operands[0]->isToken())
    Type = static_cast<CGP1Operand *>(Operands[0].get())->getToken();
  ...
  std::unique_ptr<CGP1Operand> Op = parseRegister();
  if (!Op)
    Op = parseImmediate();
  ...
  Parser.Lex(); // Eat the '['.
  std::unique_ptr<CGP1Operand> Offset = nullptr;
  if (Op)
    Offset.swap(Op);
  ...
  if (!Op) {
    if (!Offset) {
      if ((Op = parseImmediate()) && Lexer.is(AsmToken::RBrac)) {
        ...
        // SLS, the rest as RM.
        if (shouldBeSls(*Op)) {
          Operands.push_back(CGP1Operand::MorphToMemImm(std::move(Op)));
        } else {
          if (!Op->isLoImm16Signed())
            return Error(Parser.getTok().getLoc(),
                         "Memory address is not word aligned and larger than "
                         "class RM can handle");
          Operands.push_back(CGP1Operand::MorphToMemRegImm(
              CGP1::R0, std::move(Op), LPAC::ADD));
        }
        return ParseStatus::Success;
      }
    }
  ...
  if (Lexer.is(AsmToken::RBrac)) {
    ...
    if (!Offset) {
      ...
      Offset = CGP1Operand::createImm(OffsetConstExpr, Start, End);
    }
  } else {
  ...
  Operands.push_back(
      Offset->isImm()
          ? CGP1Operand::MorphToMemRegImm(BaseReg, std::move(Offset), AluOp)
          : CGP1Operand::MorphToMemRegReg(BaseReg, std::move(Offset), AluOp));
...
ParseStatus CGP1AsmParser::parseOperand(OperandVector *Operands,
                                         StringRef Mnemonic) {
...
StringRef CGP1AsmParser::splitMnemonic(StringRef Name, SMLoc NameLoc,
                                        OperandVector *Operands) {
  ...
  if (Mnemonic[0] == 'b' ||
    ...
    LPCC::CondCode CondCode =
        LPCC::suffixToCGP1CondCode(Mnemonic.substr(1, Next));
    if (CondCode != LPCC::UNKNOWN) {
      Mnemonic = Mnemonic.slice(0, 1);
      Operands->push_back(CGP1Operand::CreateToken(Mnemonic, NameLoc));
      Operands->push_back(CGP1Operand::createImm(
          MCConstantExpr::create(CondCode, getContext()), NameLoc, NameLoc));
      if (IsBRR) {
        Operands->push_back(CGP1Operand::CreateToken(".r", NameLoc));
      }
      return Mnemonic;
    }
  }
  ...
  if (Mnemonic.starts_with("sel") ||
    ...
    LPCC::CondCode CondCode = LPCC::suffixToCGP1CondCode(Mnemonic);
    if (CondCode != LPCC::UNKNOWN) {
...
static bool IsMemoryAssignmentError(const OperandVector &Operands) {
  ...
  if (CGP1Operand *PossibleAluOp =
          static_cast<CGP1Operand *>(Operands[PossibleAluOpIdx].get()))
...
static bool IsRegister(const MCParsedAsmOperand &op) {
  return static_cast<const CGP1Operand &>(op).isReg();
}
...
static bool MaybePredicatedInst(const OperandVector &Operands) {
  ...
  return StringSwitch<bool>(
             static_cast<const CGP1Operand &>(*Operands[0]).getToken())
...
bool CGP1AsmParser::parseInstruction(ParseInstructionInfo & /*Info*/,
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name == "st" &&
    ...
    Operands.insert(Operands.begin(), CGP1Operand::CreateToken("s", NameLoc));
    Operands.insert(Operands.begin() + 1,
                    CGP1Operand::createImm(
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name.starts_with("bt") &&
    ...
    Operands.insert(Operands.begin(), CGP1Operand::CreateToken("bt", NameLoc));
  }
...
LLVMInitializeCGP1AsmParser() {
  RegisterMCAsmParser<CGP1AsmParser> x(getTheCGP1Target());
}
