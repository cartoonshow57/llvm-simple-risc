//===-- CGP1TargetTransformInfo.h - CGP1 specific TTI ---------*- C++ -*-===//
//
...
// CGP1 target machine. It uses the target's detailed information to
// provide more precise answers to certain TTI queries, while letting the
// target independent and default TTI implementations handle the rest.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_CGP1_CGP1TARGETTRANSFORMINFO_H
#define LLVM_LIB_TARGET_CGP1_CGP1TARGETTRANSFORMINFO_H
...
namespace llvm {
class CGP1TTIImpl final : public BasicTTIImplBase<CGP1TTIImpl> {
  typedef BasicTTIImplBase<CGP1TTIImpl> BaseT;
  typedef TargetTransformInfo TTI;
  friend BaseT;

  const CGP1Subtarget *ST;
  const CGP1TargetLowering *TLI;

  const CGP1Subtarget *getST() const { return ST; }
  const CGP1TargetLowering *getTLI() const { return TLI; }

public:
  explicit CGP1TTIImpl(const CGP1TargetMachine *TM, const Function &F)
...
#endif // LLVM_LIB_TARGET_CGP1_CGP1TARGETTRANSFORMINFO_H
