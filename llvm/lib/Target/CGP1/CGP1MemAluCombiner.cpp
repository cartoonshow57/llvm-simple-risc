//===-- CGP1MemAluCombiner.cpp - Pass to combine memory & ALU operations -===//
//
...
//
// The CGP1 ISA supports instructions where a load/store modifies the base
...
class CGP1MemAluCombiner : public MachineFunctionPass {
public:
  static char ID;
  explicit CGP1MemAluCombiner() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override {
    return "CGP1 load / store optimization pass";
  }
...
char CGP1MemAluCombiner::ID = 0;

INITIALIZE_PASS(CGP1MemAluCombiner, DEBUG_TYPE,
                "CGP1 memory ALU combiner pass", false, false)

namespace {
bool isSpls(uint16_t Opcode) { return CGP1::splsIdempotent(Opcode) == Opcode; }
...
bool isZeroOperand(const MachineOperand &Op) {
  return ((Op.isReg() && Op.getReg() == CGP1::R0) ||
...
void CGP1MemAluCombiner::insertMergedInstruction(MachineBasicBlock *BB,
                                                  const MbbIterator &MemInstr,
...
bool isSuitableAluInstr(bool IsSpls, const MbbIterator &AluIter,
  ...
  if (Op2.isImm()) {
    ...
    if (AluIter->getOpcode() != CGP1::ADD_I_LO)
      return false;

    if (Offset.isReg() && Offset.getReg() == CGP1::R0)
      return true;
...
MbbIterator CGP1MemAluCombiner::findClosestSuitableAluInstr(
...
bool CGP1MemAluCombiner::combineMemAluInBasicBlock(MachineBasicBlock *BB) {
  bool Modified = false;
...
// machine function
bool CGP1MemAluCombiner::runOnMachineFunction(MachineFunction &MF) {
  if (DisableMemAluCombiner)
    return false;

  TII = MF.getSubtarget<CGP1Subtarget>().getInstrInfo();
  bool Modified = false;
...
FunctionPass *llvm::createCGP1MemAluCombinerPass() {
  return new CGP1MemAluCombiner();
}
