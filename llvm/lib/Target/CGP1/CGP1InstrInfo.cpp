//===-- CGP1InstrInfo.cpp - CGP1 Instruction Information ------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the CGP1 implementation of the TargetInstrInfo class.
//
//===----------------------------------------------------------------------===//
...
void CGP1InstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (!CGP1::GPRRegClass.contains(DestinationRegister, SourceRegister)) {
    llvm_unreachable("Impossible reg-to-reg copy");
  }

  BuildMI(MBB, Position, DL, get(CGP1::OR_I_LO), DestinationRegister)
      .addReg(SourceRegister, getKillRegState(KillSource))
      .addImm(0);
}

void CGP1InstrInfo::storeRegToStackSlot(
  ...
  if (!CGP1::GPRRegClass.hasSubClassEq(RegisterClass)) {
    llvm_unreachable("Can't store this register to stack slot");
  }
  BuildMI(MBB, Position, DL, get(CGP1::SW_RI))
...
void CGP1InstrInfo::loadRegFromStackSlot(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator Position,
  ...
  BuildMI(MBB, Position, DL, get(CGP1::LDW_RI), DestinationRegister)
      .addFrameIndex(FrameIndex)
      .addImm(0)
      .addImm(LPAC::ADD);
}

bool CGP1InstrInfo::areMemAccessesTriviallyDisjoint(
...
bool CGP1InstrInfo::expandPostRAPseudo(MachineInstr & /*MI*/) const {
  return false;
}
...
CGP1InstrInfo::decomposeMachineOperandsTargetFlags(unsigned TF) const {
  return std::make_pair(TF, 0u);
}

ArrayRef<std::pair<unsigned, const char *>>
CGP1InstrInfo::getSerializableDirectMachineOperandTargetFlags() const {
  using namespace CGP1II;
...
bool CGP1InstrInfo::analyzeCompare(const MachineInstr &MI, Register &SrcReg,
                                    Register &SrcReg2, int64_t &CmpMask,
                                    int64_t &CmpValue) const {
  switch (MI.getOpcode()) {
  default:
    break;
  case CGP1::SFSUB_F_RI_LO:
  case CGP1::SFSUB_F_RI_HI:
    SrcReg = MI.getOperand(0).getReg();
    SrcReg2 = Register();
    CmpMask = ~0;
    CmpValue = MI.getOperand(1).getImm();
    return true;
  case CGP1::SFSUB_F_RR:
    SrcReg = MI.getOperand(0).getReg();
...
inline static bool isRedundantFlagInstr(MachineInstr *CmpI, unsigned SrcReg,
                                        ...
                                        MachineInstr *OI) {
  if (CmpI->getOpcode() == CGP1::SFSUB_F_RR &&
      OI->getOpcode() == CGP1::SUB_R &&
  ...
  if (((CmpI->getOpcode() == CGP1::SFSUB_F_RI_LO &&
        OI->getOpcode() == CGP1::SUB_I_LO) ||
       (CmpI->getOpcode() == CGP1::SFSUB_F_RI_HI &&
        OI->getOpcode() == CGP1::SUB_I_HI)) &&
      OI->getOperand(1).getReg() == SrcReg &&
...
inline static unsigned flagSettingOpcodeVariant(unsigned OldOpcode) {
  switch (OldOpcode) {
  case CGP1::ADD_I_HI:
    return CGP1::ADD_F_I_HI;
  case CGP1::ADD_I_LO:
    return CGP1::ADD_F_I_LO;
  case CGP1::ADD_R:
    return CGP1::ADD_F_R;
  case CGP1::ADDC_I_HI:
    return CGP1::ADDC_F_I_HI;
  case CGP1::ADDC_I_LO:
    return CGP1::ADDC_F_I_LO;
  case CGP1::ADDC_R:
    return CGP1::ADDC_F_R;
  case CGP1::AND_I_HI:
    return CGP1::AND_F_I_HI;
  case CGP1::AND_I_LO:
    return CGP1::AND_F_I_LO;
  case CGP1::AND_R:
    return CGP1::AND_F_R;
  case CGP1::OR_I_HI:
    return CGP1::OR_F_I_HI;
  case CGP1::OR_I_LO:
    return CGP1::OR_F_I_LO;
  case CGP1::OR_R:
    return CGP1::OR_F_R;
  case CGP1::SL_I:
    return CGP1::SL_F_I;
  ...
  case CGP1::SRA_R:
    return CGP1::SRA_F_R;
  case CGP1::SUB_I_HI:
    return CGP1::SUB_F_I_HI;
  case CGP1::SUB_I_LO:
    return CGP1::SUB_F_I_LO;
  case CGP1::SUB_R:
    return CGP1::SUB_F_R;
  case CGP1::SUBB_I_HI:
    return CGP1::SUBB_F_I_HI;
  case CGP1::SUBB_I_LO:
    return CGP1::SUBB_F_I_LO;
  case CGP1::SUBB_R:
    return CGP1::SUBB_F_R;
  case CGP1::XOR_I_HI:
    return CGP1::XOR_F_I_HI;
  case CGP1::XOR_I_LO:
    return CGP1::XOR_F_I_LO;
  case CGP1::XOR_R:
    return CGP1::XOR_F_R;
  default:
    return CGP1::NOP;
...
bool CGP1InstrInfo::optimizeCompareInstr(
  ...
  if (SrcReg2 != 0)
    ...
    if (CmpInstr.getOpcode() == CGP1::SFSUB_F_RI_LO)
      MI = nullptr;
    else
      return false;
  }
  ...
  for (--I; I != E; --I) {
    ...
    if (Instr.modifiesRegister(CGP1::SR, TRI) ||
        Instr.readsRegister(CGP1::SR, TRI))
  ...
  if (flagSettingOpcodeVariant(MI->getOpcode()) != CGP1::NOP) {
    bool isSafe = false;
    ...
    while (!isSafe && ++I != E) {
      ...
      for (unsigned IO = 0, EO = Instr.getNumOperands(); !isSafe && IO != EO;
        ...
        if (MO.isRegMask() && MO.clobbersPhysReg(CGP1::SR)) {
          isSafe = true;
          break;
        }
        if (!MO.isReg() || MO.getReg() != CGP1::SR)
          continue;
        if (MO.isDef()) {
          isSafe = true;
          break;
...
bool CGP1InstrInfo::analyzeSelect(const MachineInstr &MI,
                                   SmallVectorImpl<MachineOperand> &Cond,
                                   unsigned &TrueOp, unsigned &FalseOp,
                                   bool &Optimizable) const {
  assert(MI.getOpcode() == CGP1::SELECT && "unknown select instruction");
  // Select operands:
  // 0: Def.
  // 1: True use.
  // 2: False use.
...
MachineInstr *
CGP1InstrInfo::optimizeSelect(MachineInstr &MI,
...
bool CGP1InstrInfo::analyzeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->getOpcode() == CGP1::BT) {
      if (!AllowModify) {
...
// reversed.
bool CGP1InstrInfo::reverseBranchCondition(
    SmallVectorImpl<llvm::MachineOperand> &Condition) const {
  assert((Condition.size() == 1) &&
         "CGP1 branch conditions should have one component.");
...
unsigned CGP1InstrInfo::insertBranch(MachineBasicBlock &MBB,
                                      MachineBasicBlock *TrueBlock,
                                      MachineBasicBlock *FalseBlock,
  ...
  if (Condition.empty()) {
    ...
    BuildMI(&MBB, DL, get(CGP1::BT)).addMBB(TrueBlock);
    return 1;
  }
  ...
  BuildMI(&MBB, DL, get(CGP1::BRCC)).addMBB(TrueBlock).addImm(ConditionalCode);
  ...
  BuildMI(&MBB, DL, get(CGP1::BT)).addMBB(FalseBlock);
  return 2;
}

unsigned CGP1InstrInfo::removeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->isDebugInstr())
      continue;
    if (Instruction->getOpcode() != CGP1::BT &&
        Instruction->getOpcode() != CGP1::BRCC) {
      break;
    }
...
Register CGP1InstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
                                             int &FrameIndex) const {
  if (MI.getOpcode() == CGP1::LDW_RI)
...
Register CGP1InstrInfo::isLoadFromStackSlotPostFE(const MachineInstr &MI,
                                                   int &FrameIndex) const {
  if (MI.getOpcode() == CGP1::LDW_RI) {
    unsigned Reg;
...
Register CGP1InstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                            int &FrameIndex) const {
  if (MI.getOpcode() == CGP1::SW_RI)
...
bool CGP1InstrInfo::getMemOperandWithOffsetWidth(
...
bool CGP1InstrInfo::getMemOperandsWithOffsetWidth(
