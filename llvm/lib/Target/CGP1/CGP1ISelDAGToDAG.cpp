//===-- CGP1ISelDAGToDAG.cpp - A dag to dag inst selector for CGP1 ------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the CGP1 target.
//
//===----------------------------------------------------------------------===//
...
#define PASS_NAME "CGP1 DAG->DAG Pattern Instruction Selection"
...
//===----------------------------------------------------------------------===//
// CGP1DAGToDAGISel - CGP1 specific code to select CGP1 machine
// instructions for SelectionDAG operations.
//===----------------------------------------------------------------------===//
namespace {

class CGP1DAGToDAGISel : public SelectionDAGISel {
public:
  CGP1DAGToDAGISel() = delete;

  explicit CGP1DAGToDAGISel(CGP1TargetMachine &TargetMachine)
...
public:
  static char ID;
  explicit CGP1DAGToDAGISelLegacy(CGP1TargetMachine &TM)
      : SelectionDAGISelLegacy(ID, std::make_unique<CGP1DAGToDAGISel>(TM)) {}
};
...
INITIALIZE_PASS(CGP1DAGToDAGISelLegacy, DEBUG_TYPE, PASS_NAME, false, false)

// Helper functions for ComplexPattern used on CGP1InstrInfo
// Used on CGP1 Load/Store instructions.
bool CGP1DAGToDAGISel::selectAddrSls(SDValue Addr, SDValue &Offset) {
  ...
  if (Addr.getOpcode() == ISD::OR &&
      Addr.getOperand(1).getOpcode() == CGP1ISD::SMALL) {
...
bool CGP1DAGToDAGISel::selectAddrRiSpls(SDValue Addr, SDValue &Base,
  ...
  if (ConstantSDNode *CN = dyn_cast<ConstantSDNode>(Addr)) {
    if (RiMode) {
      ...
      if (isInt<16>(CN->getSExtValue())) {
        ...
        Offset = CurDAG->getTargetConstant(Imm, DL, CN->getValueType(0));
        Base = CurDAG->getRegister(CGP1::R0, CN->getValueType(0));
...
bool CGP1DAGToDAGISel::selectAddrRi(SDValue Addr, SDValue &Base,
                                     SDValue &Offset, SDValue &AluOp) {
  return selectAddrRiSpls(Addr, Base, Offset, AluOp, /*RiMode=*/true);
}

bool CGP1DAGToDAGISel::selectAddrSpls(SDValue Addr, SDValue &Base,
                                       SDValue &Offset, SDValue &AluOp) {
...
namespace llvm {
namespace LPAC {
static AluCode isdToCGP1AluCode(ISD::NodeType Node_type) {
  switch (Node_type) {
  case ISD::ADD:
    return AluCode::ADD;
  case ISD::ADDE:
...
bool CGP1DAGToDAGISel::selectAddrRr(SDValue Addr, SDValue &R1, SDValue &R2,
                                     SDValue &AluOp) {
  // if Address is FI, get the TargetFrameIndex.
  ...
  ISD::NodeType AluOperator = static_cast<ISD::NodeType>(Addr.getOpcode());
  LPAC::AluCode AluCode = LPAC::isdToCGP1AluCode(AluOperator);
  if (AluCode != LPAC::UNKNOWN) {
    ...
    if (Addr.getOperand(0).getOpcode() == CGP1ISD::HI ||
        Addr.getOperand(0).getOpcode() == CGP1ISD::LO ||
        Addr.getOperand(0).getOpcode() == CGP1ISD::SMALL ||
        Addr.getOperand(1).getOpcode() == CGP1ISD::HI ||
        Addr.getOperand(1).getOpcode() == CGP1ISD::LO ||
        Addr.getOperand(1).getOpcode() == CGP1ISD::SMALL)
      return false;
...
bool CGP1DAGToDAGISel::SelectInlineAsmMemoryOperand(
...
void CGP1DAGToDAGISel::selectFrameIndex(SDNode *Node) {
  SDLoc DL(Node);
  SDValue Imm = CurDAG->getTargetConstant(0, DL, MVT::i32);
  int FI = cast<FrameIndexSDNode>(Node)->getIndex();
  EVT VT = Node->getValueType(0);
  SDValue TFI = CurDAG->getTargetFrameIndex(FI, VT);
  unsigned Opc = CGP1::ADD_I_LO;
  if (Node->hasOneUse()) {
...
FunctionPass *llvm::createCGP1ISelDag(CGP1TargetMachine &TM) {
  return new CGP1DAGToDAGISelLegacy(TM);
}
