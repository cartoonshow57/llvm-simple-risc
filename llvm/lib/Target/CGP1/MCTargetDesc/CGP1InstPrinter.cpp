//===-- CGP1InstPrinter.cpp - Convert CGP1 MCInst to asm syntax ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This class prints an CGP1 MCInst to a .s file.
//
//===----------------------------------------------------------------------===//
...
bool CGP1InstPrinter::printInst(const MCInst *MI, raw_ostream &OS,
...
static bool usesGivenOffset(const MCInst *MI, int AddOffset) {
  ...
  return LPAC::encodeCGP1AluCode(AluCode) == LPAC::ADD &&
...
bool CGP1InstPrinter::printMemoryLoadIncrement(const MCInst *MI,
                                                raw_ostream &OS,
                                                StringRef Opcode,
                                                int AddOffset) {
...
bool CGP1InstPrinter::printMemoryStoreIncrement(const MCInst *MI,
                                                 raw_ostream &OS,
                                                 StringRef Opcode,
                                                 int AddOffset) {
  if (isPreIncrementForm(MI, AddOffset)) {
...
bool CGP1InstPrinter::printAlias(const MCInst *MI, raw_ostream &OS) {
  switch (MI->getOpcode()) {
  case CGP1::LDW_RI:
...
void CGP1InstPrinter::printInst(const MCInst *MI, uint64_t Address,
...
void CGP1InstPrinter::printMemImmOperand(const MCInst *MI, unsigned OpNo,
                                          raw_ostream &OS) {
  const MCOperand &Op = MI->getOperand(OpNo);
  if (Op.isImm()) {
    OS << '[' << formatHex(Op.getImm()) << ']';
...
void CGP1InstPrinter::printHi16ImmOperand(const MCInst *MI, unsigned OpNo,
                                           raw_ostream &OS) {
  const MCOperand &Op = MI->getOperand(OpNo);
  if (Op.isImm()) {
...
void CGP1InstPrinter::printHi16AndImmOperand(const MCInst *MI, unsigned OpNo,
                                              raw_ostream &OS) {
  const MCOperand &Op = MI->getOperand(OpNo);
  if (Op.isImm()) {
...
void CGP1InstPrinter::printLo16AndImmOperand(const MCInst *MI, unsigned OpNo,
                                              raw_ostream &OS) {
  const MCOperand &Op = MI->getOperand(OpNo);
  if (Op.isImm()) {
...
static void printMemoryBaseRegister(raw_ostream &OS, const unsigned AluCode,
  ...
  if (LPAC::isPreOp(AluCode))
    OS << "*";
  OS << "%" << CGP1InstPrinter::getRegisterName(RegOp.getReg());
...
void CGP1InstPrinter::printMemRiOperand(const MCInst *MI, int OpNo,
                                         raw_ostream &OS) {
...
void CGP1InstPrinter::printMemRrOperand(const MCInst *MI, int OpNo,
                                         raw_ostream &OS) {
  const MCOperand &RegOp = MI->getOperand(OpNo);
  const MCOperand &OffsetOp = MI->getOperand(OpNo + 1);
  const MCOperand &AluOp = MI->getOperand(OpNo + 2);
  ...
  if (LPAC::isPostOp(AluCode))
    OS << "*";
  OS << " " << LPAC::cgp1AluCodeToString(AluCode) << " ";
  OS << "%" << getRegisterName(OffsetOp.getReg());
  OS << "]";
}

void CGP1InstPrinter::printMemSplsOperand(const MCInst *MI, int OpNo,
                                           raw_ostream &OS) {
  const MCOperand &RegOp = MI->getOperand(OpNo);
  const MCOperand &OffsetOp = MI->getOperand(OpNo + 1);
  const MCOperand &AluOp = MI->getOperand(OpNo + 2);
...
void CGP1InstPrinter::printCCOperand(const MCInst *MI, int OpNo,
                                      raw_ostream &OS) {
  LPCC::CondCode CC =
  ...
  if (CC >= LPCC::UNKNOWN)
    ...
    OS << cgp1CondCodeToString(CC);
}

void CGP1InstPrinter::printPredicateOperand(const MCInst *MI, unsigned OpNo,
                                             raw_ostream &OS) {
  LPCC::CondCode CC =
      static_cast<LPCC::CondCode>(MI->getOperand(OpNo).getImm());
  // Handle the undefined value here for printing so we don't abort().
