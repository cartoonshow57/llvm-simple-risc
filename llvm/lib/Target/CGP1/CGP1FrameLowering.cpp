//===-- CGP1FrameLowering.cpp - CGP1 Frame Information ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the CGP1 implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
void CGP1FrameLowering::determineFrameLayout(MachineFunction &MF) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  const CGP1RegisterInfo *LRI = STI.getRegisterInfo();
...
// ADJDYNALLOC pseudo instructions with a CGP1:ADDI with the
// maximum call frame size as the immediate.
void CGP1FrameLowering::replaceAdjDynAllocPseudo(MachineFunction &MF) const {
  const CGP1InstrInfo &LII =
      *static_cast<const CGP1InstrInfo *>(STI.getInstrInfo());
  unsigned MaxCallFrameSize = MF.getFrameInfo().getMaxCallFrameSize();

  for (MachineBasicBlock &MBB : MF) {
    for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
      if (MI.getOpcode() == CGP1::ADJDYNALLOC) {
        DebugLoc DL = MI.getDebugLoc();
        Register Dst = MI.getOperand(0).getReg();
        Register Src = MI.getOperand(1).getReg();

        BuildMI(MBB, MI, DL, LII.get(CGP1::ADD_I_LO), Dst)
            .addReg(Src)
...
void CGP1FrameLowering::emitPrologue(MachineFunction &MF,
  ...
  // Push old FP
  // st %fp,-4[*%sp]
  BuildMI(MBB, MBBI, DL, LII.get(CGP1::SW_RI))
      .addReg(CGP1::FP)
      .addReg(CGP1::SP)
      .addImm(-4)
  ...
  // Generate new FP
  // add %sp,8,%fp
  BuildMI(MBB, MBBI, DL, LII.get(CGP1::ADD_I_LO), CGP1::FP)
      .addReg(CGP1::SP)
      .addImm(8)
      .setMIFlag(MachineInstr::FrameSetup);
  ...
  // sub %sp,StackSize,%sp
  if (StackSize != 0) {
    BuildMI(MBB, MBBI, DL, LII.get(CGP1::SUB_I_LO), CGP1::SP)
        .addReg(CGP1::SP)
        .addImm(StackSize)
        .setMIFlag(MachineInstr::FrameSetup);
  }
...
void CGP1FrameLowering::emitEpilogue(MachineFunction & /*MF*/,
  ...
  BuildMI(MBB, MBBI, DL, LII.get(CGP1::ADD_I_LO), CGP1::SP)
      .addReg(CGP1::FP)
      .addImm(0);

  // Restore the frame pointer from the stack.
  BuildMI(MBB, MBBI, DL, LII.get(CGP1::LDW_RI), CGP1::FP)
      .addReg(CGP1::FP)
      .addImm(-8)
      .addImm(LPAC::ADD);
}
...
void CGP1FrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  MachineFrameInfo &MFI = MF.getFrameInfo();
  const CGP1RegisterInfo *LRI =
      static_cast<const CGP1RegisterInfo *>(STI.getRegisterInfo());
  int Offset = -4;
